...
目前没有录制、剪辑、项目管理、导出的 CLI。使用当前环境提供的浏览器工具，先读其操作说明。实际读页面后选按钮，不保存或复用旧 accessibility 索引。

已验证流程：Select file 导入 → 面板调整 → Export 选择格式/质量 → 等待完成 → 检查实际文件。已实测短视频 H.264 和 PNG 导出，以及视频刷新恢复；未完成全部设备模型、声音、4K/GIF/透明 WebM 的测试。

视频导出建议在 Chrome 验证；保持录制/导出标签可见。原生保存或共享窗口不属于网页 DOM。录屏使用 getDisplayMedia，可能需要用户选择来源和授予权限；当前尚未建立无人值守录制链路。
