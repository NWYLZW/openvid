...
export function CameraKeyframeEditor({fragment,onUpdate,onDelete,onClose,onSeek,currentTime=0,children}:Props) {
  ...
  return <div className="flex h-full flex-col text-foreground">
    <div className="flex items-center gap-2 border-b border-border p-3"><DetailPageHeader label="Camera keyframes" icon="ph:arrow-left-bold" onBack={onClose}/><button className="ml-auto text-xs text-destructive" onClick={onDelete}>Delete motion</button></div>
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      <p className="text-xs text-muted-foreground">Select a keyframe to preview and adjust its camera pose. Changes are saved in this project.</p>
      ...
      <section className="space-y-3 rounded border border-border p-3" aria-label="Selected camera keyframe">
        ...
        <DirectionPad angleX={frame.pitch} angleY={frame.yaw} maxAngle={65} onChange={(pitch,yaw)=>update({pitch,yaw})} label="3D tilt" accentRgb="249,115,22"/>
