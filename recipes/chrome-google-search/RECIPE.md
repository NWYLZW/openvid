...
当前：逐字输入2倍速版，约7.4秒、1080p60fps。34字符真实输入素材的打字段2倍速，约2.3秒；输入后稍停，鼠标与镜头0.6秒到搜索按钮，点击后等0.4秒再拉远，结果页结束。

## 本次入口

- 当前参数：[typing-2x-edit.json](typing-2x-edit.json)。
- 恢复工作先读 `runs/typing-2x-2026-09-10/run.md`，核对真实工程和产物。
- 源素材为该运行typing-2x-source.mp4，来自Openvid Record真实键盘输入；去除左侧工具指针所在空白边，光标/水波/镜头在工程内渲染。
...
Zoom轨道和面板可直接看/改Camera zoom，和Motion共用实际参数，不是新增重复缩放。
