...
鼠标使用余弦缓动，提交镜头使用[0.37,0,0.63,1]近似相同缓入缓出；起点和终点严格同步，中间曲线近似。已检查共享节拍和配置重放一致，后续search-only短版已实际导出并检查同步过程。

## 搜索即结束（当前成片范围）

用户取消后续滚动、图片和文章浏览。使用synced-quick版源的0–7.5秒，配置search-only-edit.json；相机6.5秒回正，停留1秒结束，结果出现约4.83秒。保留1.5秒内输入和2.9–4.35秒鼠标/镜头同步移动，点击约4.57秒。先前Mac锁屏已解除，本次通过原3094编辑器导出，具体状态见runs/search-only-2026-09-10/run.md。
