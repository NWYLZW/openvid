...
录制 Raft PWA 且需避免 Chrome 调试提示条时，先读 [独立进程 PWA 录制](pwa-recording.md)。已实际跑通；下面旧记录中的 PWA 来源限制属于历史测试，不再作为普遍结论。
...
系统选择器AX表格可能显示行文本，但原生AXChildren/AXRows读不到行，按标题AXPress没有效果。必须检查“selected Raft”与分享按钮已启用再分享，不能把无错误退出当成选中。在当前Chrome桌面上，通过窗口截图和实时CGWindowBounds换算鼠标位置可选择行；本次截图包含Chrome父窗口且去掉菜单栏，不能把窗口截图像素直接当全局坐标。固定坐标不可跨次复用。

共享开始后页面高度会因共享提示改变（本次856→799 CSS px）；点击前重新测量发送框并验证document.activeElement，不沿用录制前坐标。原片3354×2234是稀疏VFR屏幕视频；拼接前先fps=60再trim，反过来会导致时长和输入事件错位。原片含固定工具指针，本次仅复制同一帧邻近空白背景清理该区域；必须检查区域全程没有应用内容，并归档原片、矩形和处理说明。它不等于原始无光标采集。

中文可通过已验证的type-humanly逐字符pressSequentially输入，本次18字符约2.4秒；这不是完整拼音输入法候选过程。只录制草稿，发送消息需有明确授权。实际制作记录在Raft配方，录制UI变化、鼠标设计轨迹和实际点击时刻分别归档。
